...
{ 
  # Ultima correcao: 07/07/21 - adicionado xreg
  nef=n+burn.in
  model = list(dfrac=d)
...
loglik_fixed <- function(par, p, q, b, y, m, family="Gstudent", index1, index2=NULL, xreg=NULL , fixed.ma, fixed.ar, fit.d, fit.alpha )
{
  ...
  if (p == 0) {phi = 0
  } else { #  AR presente no modelo
    if(len.fixed.ar==0) { phi <- par[1:p] # AR não está fixo
    } else{ # se AR está fixo
      phi <- fixed.ar
  ...
  if (q == 0) {
    theta = 0
  } else { # MA presente no modelo
    if(len.fixed.ma==0){ # se MA está livre
      if(len.fixed.ar==0) {theta <- par[p+(1:q)] # MA livre e AR livre
      }else{theta <- par[len.fixed.ar+(1:q)]  } # MA livre mas AR está fixo (muda o tamanho do par)
    }else{
      theta <- fixed.ma
...
symarfima.fit <- function(y, p = NULL, q = NULL,  family = "Gstudent", start.Sym = NULL, index1 = NULL, index2 = NULL, xreg = NULL, 
                          ...
                          h.method=1) 
{
  # ultima correcao: 07/07/21 - xreg corrigido e testado  
  # no futuro temos que testar se q == length(fixed.ma) etc
  # parâmetro dentro da função: (phi, theta, d, beta, alpha, varphi)
  # cons = logical, should be the optimization performed with constrains?
  ...
  if(is.null(start.Sym)){
    if(is.null(fixed.ar) & is.null(fixed.ma) & fit.d & fit.alpha){
      ...
      if (is.null(start.Sym))  start.Sym = c(rep(0,p+q), 0.1, rep(0,b), mean(y), 1) #modificado 07/07
    }else{
      ...
      if(fit.d) start.Sym = c(start.Sym,0.1) # reverti aqui com o xreg
  ...
  # garantindo que o chute inicial tem loglike definido
  ...
  if (p == 0) {phi = 0
    ...
    if(is.null(fixed.ar)) { phi <- par[1:p] # AR não está fixo
    } else{ # se AR está fixo
      phi <- fixed.ar
      phi[where.fix.ar] = par[1:len.fixed.ar]
    }
  ...
  if (q == 0) {
    ...
    if(is.null(fixed.ma)){ # se MA está livre
      if(is.null(fixed.ar)) {theta <- par[p+(1:q)] # MA livre e AR livre
      }else{theta <- par[len.fixed.ar+(1:q)]  } # MA livre mas AR está fixo (muda o tamanho do par)
    }else{
      theta <- fixed.ma
      where.fix.ma = which(is.na(fixed.ma))
  ...
  if(!is.null(h)){
    y.ar = c(y[n+((1-p):0)],rep(0,h)) # p últimos de xreg para economizar memória
    r.ck = c(y[n+((1-m):0)]-mut[n+((1-m):0)],rep(0,h))
    y.prev = rep(0,h)
    if(is.null(xreg)){ 
      hvx = c(rep(0, p+h))
    }else{
      hxreg <- matrix(rbind(xreg[n+((1-p):0)], xreg_hat),ncol = b) # p últimos de xreg para economizar memória
      hvx = hxreg%*%beta
    }
  ...
  if("try-error"%in%class(vcov1)){
    ...
    zstat1 <- 666
    pvalues1 = 2
    coef1 = fit$par[1:ss.len]# coeficientes
  }
  else{
    sd1 <- sqrt(abs(diag(vcov1)))
    coef1 = fit$par[1:ss.len]# coeficientes
    zstat1 <- coef1/sd1 #estat de Wald
  ...
  model_presentation1 <-cbind(round(coef1,5),round(sd1,5),round(zstat1,5),pvalues1) #o que vai retornar
  colnames(model_presentation1)<-c("Estimate","Std. Error","z value","Pr(>|z|)") #nomes do que vai retornar
...
vc.f <- function(d, theta = NA, infinite = 100)
{
  ...
  pik<-c(1,rep(0,infinite))  # implementacao dos pi_k's da formula 3 dentro da funcao
  if(d != 0){ 
    pik[2]= d
    for(j in 3:(infinite+1)){
      pik[j]<- ((j-2+d)/(j-1) )*pik[j-1] # acerto para o vetor comecando no zero
    }
  }
  
  ck= c(1, rep(0,infinite))
  for(i in 2:(infinite+1))
    ck[i]=sum(rev(pik[1:i])*vtheta[1:i])  #ck[i] eh equivalente a c_(i-1)
...
const = function(t,mk,plk)
{
  ...
  return(ymx) #retorna o vetor y_{t-1}, \cdots, t_{t-k} nessa ordem
}