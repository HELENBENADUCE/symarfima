...
loglik_fixed <- function(par, p, q, b, y, m, family="Gstudent", index1, index2=NULL, xreg=NULL , fixed.ma, fixed.ar, fit.d, fit.alpha )
{
  ...
  if (p == 0) {phi = 0
    ...
    if(len.fixed.ar==0) { phi <- par[1:p] # AR not is fix
    } else{ # if AR is fix
      phi <- fixed.ar
  ...
  if (q == 0) {
    ...
    if(len.fixed.ma==0){ # if MA
      ...
      }else{theta <- par[len.fixed.ar+(1:q)]  } # MA but AR is fix
    }else{
      theta <- fixed.ma
                          ...
                          h.method=1) 
{
  ...
  #  kickoff has log-likelihood defined
